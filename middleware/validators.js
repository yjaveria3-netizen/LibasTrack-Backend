...
const brandUpdateValidation = [
  ...
  body('logo').optional().trim().if(val => val && val.length > 0).isURL().withMessage('Logo must be a valid URL'),
  ...
  body('website').optional().trim().if(val => val && val.length > 0).isURL().withMessage('Website must be a valid URL'),
